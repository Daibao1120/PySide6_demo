...
class DataVisualizer(QMainWindow):
    ...
    def display_data(self, selected_keys):
        if not selected_keys:
            # 可以選擇在這裡顯示錯誤消息或直接返回
            self.alert_text_box.setText("No data selected for display.")
            return
        self.selected_keys = selected_keys
        self.max_frames = max(
            len(self.collecteds_data[key]['x_axis_labels']) for key in selected_keys)
        ...
        def update(frame):
            ...
            for key in selected_keys:
                data = self.collecteds_data[key]
                dates = [datetime.strptime(d, "%Y/%m/%d %H:%M:%S.%f")
                         for d in data['x_axis_labels']][:frame+1]
                values = data['y_axis_labels'][:frame+1]
        ...
        self.ani = FuncAnimation(
            self.canvas.figure, update, frames=self.max_frames, interval=100, repeat=False)
        self.canvas.draw()
